...
import { Trash2, Plus, Minus, ShoppingBag, ChevronRight, Loader2, Tag } from 'lucide-vue-next'
...
async function handleSubmit() {
  if (!form.value.user.name || !form.value.user.email || !form.value.user.tel || !form.value.user.address) {
  ...
  <div class="max-w-7xl mx-auto px-4 py-8">
    ...
    <div v-else class="grid lg:grid-cols-3 gap-8">
          ...
          <div class="flex-1 min-w-0">
            ...
            <div class="flex items-center gap-3 mt-2">
              <div class="flex items-center border border-gray-300 rounded-lg overflow-hidden">
                <button
                  @click="cart.updateItem(item.id, item.product_id, Math.max(1, item.qty - 1))"
                ...
                <button
                  @click="cart.updateItem(item.id, item.product_id, item.qty + 1)"
                  class="px-2 py-1 hover:bg-gray-100 cursor-pointer"
                >
                  <Plus :size="14" />
                </button>
      ...
      <div class="space-y-4">
        ...
        <!-- Contact Form -->
        <div class="bg-white rounded-xl border border-gray-200 p-5">
          <h3 class="font-semibold text-gray-900 mb-4">聯絡資料</h3>
          <div class="space-y-3">
            <div>
              <label class="block text-xs font-medium text-gray-700 mb-1">姓名 *</label>
              <input v-model="form.user.name" placeholder="請輸入姓名" class="w-full h-9 border border-gray-300 rounded-md px-3 text-sm focus:outline-none focus:ring-2 focus:ring-red-500" />
            </div>
            <div>
              <label class="block text-xs font-medium text-gray-700 mb-1">Email *</label>
              <input v-model="form.user.email" type="email" placeholder="請輸入 Email" class="w-full h-9 border border-gray-300 rounded-md px-3 text-sm focus:outline-none focus:ring-2 focus:ring-red-500" />
            </div>
            <div>
              <label class="block text-xs font-medium text-gray-700 mb-1">電話 *</label>
              <input v-model="form.user.tel" placeholder="請輸入電話" class="w-full h-9 border border-gray-300 rounded-md px-3 text-sm focus:outline-none focus:ring-2 focus:ring-red-500" />
            </div>
            <div>
              <label class="block text-xs font-medium text-gray-700 mb-1">地址 *</label>
              <input v-model="form.user.address" placeholder="請輸入地址" class="w-full h-9 border border-gray-300 rounded-md px-3 text-sm focus:outline-none focus:ring-2 focus:ring-red-500" />
            </div>
            <div>
              <label class="block text-xs font-medium text-gray-700 mb-1">備註</label>
              <textarea v-model="form.message" placeholder="特殊需求或備註" rows="2" class="w-full border border-gray-300 rounded-md px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-red-500 resize-none" />
            </div>
          </div>
        </div>
        ...
        <button
          @click="handleSubmit"
          :disabled="submitting"
          class="w-full flex items-center justify-center gap-2 bg-red-700 hover:bg-red-800 text-white py-3 rounded-xl font-semibold text-lg transition-colors disabled:opacity-60 cursor-pointer"
        >
